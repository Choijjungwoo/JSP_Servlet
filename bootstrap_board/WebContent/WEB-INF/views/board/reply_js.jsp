...
<script>
	...
	getPage("<%=request.getContextPath()%>/replies/list?bno=${board.bno}&page="+replyPage);
	...
	$('#replyModBtn').on('click',function(event){
		...
		$.ajax({
			url:"<%=request.getContextPath()%>/replies/modify",
			type:"post",
	...
	$('#replyDelBtn').on('click',function(event){
		...
		$.ajax({
			url:"<%=request.getContextPath()%>/replies/remove",
			type:"post",
			data:JSON.stringify(sendData),
			success:function(result){
				if(result=="SUCCESS"){
